...
function NuevoReview() {
    ...
    const idReview = searchParams.get('idReview')
    ...
    useEffect(() => {
        const obtenerDatos = async () => {
            // Obtener los datos de la media
            const dirBusqueda = tipoMedia == "musica" ? `${urlPorMedia[tipoMedia]}/album/${idMedia}` : `${urlPorMedia[tipoMedia]}/${idMedia}`;
            const mediaResponse = await fetch(dirBusqueda);
            const mediaData = await mediaResponse.json();
            setMediaInfo(mediaData);
    
            try {
                // Obtener la review existente, si hay
                const reviewResponse = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}api/users/${session.data.user.id}/review/${idReview}`);
                if (reviewResponse.ok) {
                    ...
                    setRating(reviewData.calificacion); // Actualiza el estado de rating aquí
                    setagregadas(reviewData.mediaRecuerda);
                } else {
                    setReview({
                        titulo: mediaData.nombre,
                        descripcion: '',
                        calificacion: 0,
                    });
                    setRating(0); // Establece la calificación a 0 si no hay una review existente
                }
            } catch (error) {
    ...
    const crearReseña = async () => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}api/users/${session.data.user.id}/reviews/${idReview}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${session.data.user.token}`
                },
                body: JSON.stringify({media:reviewData.media,descripcion:reviewData.descripcion,calificacion:reviewData.calificacion,mediaRecuerda:reviewData.mediaRecuerda})
            });
                        ...
                        <div className="d-flex flex-wrap">
                            ...
                            {selectedItems.map((item, index) => (
                                <div key={index} className="mb-3 me-3" style={{ width: '135px' }}>
                                    ...
                                    />
                                    <div className="text-center mt-2" style={{ fontSize: '1.2rem' }}>{item.nombre || item.titulo}</div>
                                </div>
                            ))}
